...
export const TrendingStats = ({state}: any) => {
	let data = state?.trendingStats;
	...
	if (!state?.trendingStats?.length) return <></>;

	return (
		<>
			<h1 style={{width: '100%', textAlign: 'center', color: theme === 'dark' ? '#9eff00' : '#427a4c'}}>Trending stats</h1>
			<div style={{width: '45%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Floor change(eth price)</p>
...
export const TrendingStatsMemo = React.memo(TrendingStats, memoProps);
