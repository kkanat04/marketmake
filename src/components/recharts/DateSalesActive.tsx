...
import {
	...
	XAxis,
	YAxis} from 'recharts';
import { SET_DATA_SALES_ACTIVE } from 'redux/slicers/ProfileSlicer';

import {memoProps} from '../../utils/memoProps';

export const DateSalesActive = ({state, data}: any) => {
	const dispatch = useAppDispatch();
	const {theme} = useAppSelector(stateRedux => stateRedux.profile);

	useEffect(() => {
		dispatch(SET_DATA_SALES_ACTIVE({data: state?.date2salesActive}));
	}, [state]);

	if (!state?.date2salesActive) return <></>;
	return (
		<>
			<div style={{width:'95%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Sales per day (+ unique tokenIds per day) active</p>
			...
			<div style={{width:'45%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Repeated token ids active</p>
				<ResponsiveContainer width="100%" height="100%">
					<AreaChart
						data={data}
					>
			...
			<div style={{width:'45%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Unique wallets sells active</p>
				<ResponsiveContainer width="100%" height="100%">
					<AreaChart
						data={data}
					>
			...
			<div style={{width:'45%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Volume (active)</p>
				<ResponsiveContainer width="100%" height="100%">
					<AreaChart
						data={data}
					>
			...
			<div style={{width:'45%', height: '50vh', textAlign: 'center'}}>
				<p style={{color: theme === 'light' ? '#000' : '#fff'}}>Price (active)</p>
				<ResponsiveContainer width="100%" height="100%">
					<AreaChart
						data={data}
					>
...
export const DateSalesActiveMemo = React.memo(DateSalesActive, memoProps);
